...
class ReportController extends BaseApiController
{
    ...
    /**
     * ✅ Helper: Format loan prices with company names
     */
            ...
            if (is_array($prices)) {
                foreach ($prices as $item) {
                    if (isset($item['company_id']) && isset($item['price'])) {
                        $companyObj = Company::find($item['company_id']);
                        $loanPrices[] = [
                            'company_id'   => $item['company_id'],
                            'company_name' => $companyObj ? $companyObj->company_name : $item['company_id'],
    ...
    /**
     * ✅ WAREHOUSE STOCK DATA – uses product's own SKU column
     */
    private function warehouseStockData(Request $request)
    {
        ...
        try {
            ...
            $warehouseQuery = Warehouse::where('status', 'active');
            if ($warehouseId) {
            ...
            if ($warehouses->isEmpty()) {
                return $this->errorResponse('No active warehouses found', 404);
            }
            ...
            foreach ($warehouses as $warehouse) {
                ...
                if ($firstWarehouse === null) {
                    $firstWarehouse = $warehouseInfo;
                }

                $inventoryItems = Inventory::where('warehouse_id', $warehouse->warehouse_id)->get();

                $productIds = [];
                foreach ($inventoryItems as $item) {
                    if (!empty($item->product_ids) && is_array($item->product_ids)) {
                        $productIds = array_merge($productIds, $item->product_ids);
                ...
                if ($productId) {
                    $productIds = array_intersect($productIds, [$productId]);
                }
                ...
                foreach ($products as $product) {
                    ...
                    $catKey = ($category->category_id ?? 'uncategorized') . '_' . $warehouse->warehouse_id;
    ...
    /**
     * COLLECTION CENTER STOCK DATA – uses product's own SKU column
     */
    ...
    /**
     * STOCK MOVEMENT REPORT – uses product's own SKU column
     */
    public function stockMovementReport(Request $request)
    {
        ...
        try {
            ...
            foreach ($movements as $movement) {
                ...
                $loanPrices = $product ? $this->formatLoanPrices($product) : [];
    ...
    /**
     * OWNER STOCK DATA – uses product's own SKU column
     */
    ...
    public function purchasesReport(Request $request)
    {
        ...
        try {
            ...
            // ... rest of purchases report (unchanged) ...
            $processedPurchases = [];
            $totalAmount = 0;
            $totalQuantity = 0;
            $supplierSummary = [];
            ...
            foreach ($purchases as $purchase) {
                ...
                if (empty($modelName) && $category) $modelName = $category->model ?? '';
                $processedPurchases[] = [
    ...
    /**
     * CLEAR SALES REPORT – uses product's own SKU column
     */
    public function clearSalesReport(Request $request)
    {
        ...
        try {
            ...
            $query = Sale::with(['agent', 'customer', 'product.category', 'company'])
            ...
            foreach ($sales as $sale) {
                ...
                $saleObject = [
                    ...
                    'product' => $product ? [
                        ...
                        'cash_selling_price' => floatval($product->cash_selling_price ?? 0),
                        'loan_prices' => $loanPrices,
                        'loan_selling_price' => !empty($loanPrices) ? $loanPrices[0]['price'] : 0,
                        'stock_status' => $product->stock_status,
                        'sku' => $productSku,
                        'category' => $category ? [
                            'category_id' => $category->category_id,
    ...
    /**
     * Get top products from sales collection – uses product's own SKU column
     */
