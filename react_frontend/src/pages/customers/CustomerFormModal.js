...
export default function CustomerFormModal({ open, onClose, customer }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (customer) {
                await update(customer.id, form); // customer.id exists because we mapped it in the hook
            ...
            <form onSubmit={handleSubmit}>
                ...
                <DialogContent>
                    <Box display="flex" flexDirection="column" gap={2} mt={1}>
                        <TextField label="Full Name" name="customer_name" value={form.customer_name} onChange={handleChange} required fullWidth />
                        <TextField label="NIDA" name="nida" value={form.nida} onChange={handleChange} fullWidth />
                        <TextField label="Phone (MSISDN)" name="msisdn" value={form.msisdn} onChange={handleChange} fullWidth />
